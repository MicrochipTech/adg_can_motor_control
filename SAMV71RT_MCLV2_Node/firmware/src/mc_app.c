...
#ifdef USE_PUSH_BUTTON
        VelRefRaw = (float)((float)potReading * POT_ADC_COUNT_FW_SPEED_RATIO);
#else
        VelRefRaw = (float)((float)canSetpoint * POT_ADC_COUNT_FW_SPEED_RATIO);
#endif
        /* LPF */
...
#if defined(MCLV2) && defined(USE_REVERSE_DIR)
/******************************************************************************/
...
/******************************************************************************/
/* Function name: MC_APP_MC_GetSpeed                                          */
/* Function parameters:                                                       */
/*      - speedMin: Minimum rotor angular speed in RPM                        */
/*      - speedMax: Maximum rotor angular speed in RPM                        */
/* Function return: Rotor speed index [0, 255]                               */
/* Description: Get the estimated electric rotor angular speed index          */
/******************************************************************************/
uint8_t MC_APP_MC_GetSpeed(float speedMin, float speedMax)
{
    float WeHat = speedData.WeHat;

    /* Estimated electric rotor angular speed */
    if ((mc_appData.mcState == MC_APP_MC_STATE_RUNNING) && (WeHat > 0))
    {
        /* Ensure that the speed is within the specified boundaries */
        if (WeHat > speedMax)
        {
            WeHat = speedMax;
        }
        ...
        /* return the speed on a [0, 255] scale */
        return (uint8_t)((WeHat - speedMin) * 255 / (speedMax - speedMin));
    }
    else
    {
...
/******************************************************************************/
/* Function name: MC_APP_MC_GetTarget                                         */
/* Function parameters: None                                                  */
/* Function return: ADC measurement of the pot [0, 4096] scaled to [0, 255]   */
/* Description: Returns the position of the potentiometer [0, 255]            */
/******************************************************************************/
uint8_t MC_APP_MC_GetPot(void)
{
    return (uint8_t)(potReading / 16);
}

/******************************************************************************/
/* Function name: MC_APP_MC_SetTarget                                         */
/* Function parameters:                                                       */
/*      - setpointIndex: Target speed index [0, 255]                          */
/*      - setpointMin: Setpoint corresponding to the minimum rotor angular    */
/*                     speed [0, 4096[                                        */
/*      - setpointMax: Setpoint corresponding to the maximum rotor angular    */
/*                     speed ]0, 4096]                                        */
/* Function return:                                                           */
/* Description: Simulate the ADC measurement equivalent of the control        */
/*              potentiometer measurement defining the speed target           */
/******************************************************************************/
void MC_APP_MC_SetTarget(uint8_t setpointIndex, unsigned short setpointMin, unsigned short setpointMax)
{
    if (setpointIndex > 0)
    {
        /* Scale the setpoint index */
        canSetpoint = (short)((float)setpointIndex * (setpointMax - setpointMin) / 255 + setpointMin);
    }
    else
    {
        /* Stop the rotor */
...
void MC_APP_MC_Tasks ( void )
{
    ...
    switch (mc_appData.mcState)
    {
        ...
        case MC_APP_MC_STATE_INIT:
        {
            ...
            if (canSetpoint > 0)
            {
                mc_appData.mcState = MC_APP_MC_STATE_START;
            }
#endif
            break;
        }
